...
import com.LOLdaojucheng.common.Const.PaymentPlatForEnum;
import com.LOLdaojucheng.common.ServerResponse;
import com.LOLdaojucheng.dao.*;
import com.LOLdaojucheng.pojo.*;
...
import com.LOLdaojucheng.utils.FTPUtils;
import com.LOLdaojucheng.utils.PropertiesUtils;
import com.LOLdaojucheng.vo.CartOrderItemVO;
import com.LOLdaojucheng.vo.OrderItemVo;
import com.LOLdaojucheng.vo.OrderVO;
import com.LOLdaojucheng.vo.ShippingVO;
...
public class OrderServiceImpl implements IOrderService {
    ...
    @Override
    @Transactional
    public ServerResponse createOrder(Integer userId, Integer shippingId,BigDecimal totalPrice) {
        //参数非空校验
        if (shippingId==null){
            return ServerResponse.serverResponseByError("地址参数不能为空");
        }
        //根据userId查询购物车中已被选中的商品-->ListCart
...
//        orderTotalPrice = getTotalPrice(orderItemList);
        orderTotalPrice = totalPrice;
        //创建订单order并将其保存到数据库
        Order order = createNewOrder(userId,shippingId,orderTotalPrice);
        if (order==null){
    ...
    private OrderVO getOrderVO(Order order,List<OrderItem> orderItemList,Integer shippingId){
        ...
        Shopping shopping = shoppingMapper.selectByPrimaryKey(shippingId);
        if (shopping!=null){
            orderVO.setShippingId(shippingId);
            ShippingVO shippingVO = assembleShippingVO(shopping);
        ...
        Const.ORDER_STATUS order_status = Const.ORDER_STATUS.codeOf(orderVO.getStatus());
        if (order_status!=null){
            orderVO.setStatusDesc(order_status.getDesc());
        }
        orderVO.setPostage(0);
        orderVO.setPayment(order.getPayment());
        orderVO.setPaymentType(order.getPaymentType());
        Const.PAY_MENT PAY_MENT= Const.PAY_MENT.codeOf(orderVO.getStatus());
        if (PAY_MENT!=null){
            orderVO.setStatusDesc(PAY_MENT.getDesc());
        }
    ...
    /***
     * List<OrderItem>-->List<OrderItemVI>
     */
    private OrderItemVo assembleOrderItemVO(OrderItem orderItem){
        OrderItemVo orderItemVo = new OrderItemVo();
        if (orderItem!=null){
            orderItemVo.setQuantity(orderItem.getQuantity());
            orderItemVo.setCreateTimel(DateUtils.DateToString(orderItem.getCreateTime()));
     ...
     * 计算订单价格
     */
    private BigDecimal getTotalPrice(List<OrderItem> orderItemLists){
        BigDecimal bigDecimal = new BigDecimal(0);
        for (OrderItem orderItem:
             orderItemLists) {
            bigDecimal = BigDeccimalUtils.add(bigDecimal.doubleValue(),orderItem.getTotalPrice().doubleValue());
        }
        return bigDecimal;
    }
     ...
     */
    private Order createNewOrder(Integer userId, Integer shippingId, BigDecimal orderTotalPrice){
        Order order = new Order();
        order.setOrderNo(generateOrderNo());
        order.setUserId(userId);
        order.setShippingId(shippingId);
        order.setStatus(Const.ORDER_STATUS.ORDER_UN_PAY.getCode());
        //设置订单金额
        order.setPayment(orderTotalPrice);
        order.setPostage(0);
        order.setPaymentType(Const.PAY_MENT.ONLINE.getCode());
        //保存订单
    ...
    private  ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        for (Cart cart:
            ...
            if (product.getStatus()==Const.Product_state.STATE_OFFLINE.getCode()||product.getStatus()==Const.Product_state.STATE_DELECT.getCode()){
                return ServerResponse.serverResponseByError("id为"+cart.getProductId()+"商品已被下架或者删除");

            }
            if (product.getStock()<cart.getQuantity()){//库存不足
                return ServerResponse.serverResponseByError("id为"+cart.getProductId()+"库存不足");
            }
            orderItem.setQuantity(cart.getQuantity());
            orderItem.setCurrentUnitPrice(product.getPrice());
            orderItem.setProductId(product.getId());
            orderItem.setProductImage(product.getSubImages());
            orderItem.setProductName(product.getName());
            orderItem.setTotalPrice(BigDeccimalUtils.mul(product.getPrice().doubleValue(),cart.getQuantity().doubleValue()));
 ...
 */
        ...
        public ServerResponse cancel(Integer userId,Long orderNo){
            //参数非空校验
            if (orderNo==null){
                return ServerResponse.serverResponseByError("订单号为空");
            }
            ...
            //判断订单状态
            System.out.print(order.getStatus()!=Const.ORDER_STATUS.ORDER_UN_PAY.getCode());
            if(order.getStatus()!=Const.ORDER_STATUS.ORDER_UN_PAY.getCode()){
                return ServerResponse.serverResponseByError("订单不可取消");
            }

        //修改订单状态
        order.setStatus(Const.ORDER_STATUS.ORDER_CANCELED.getCode());
    ...
    public ServerResponse get_order_cart_product(Integer userId){
        ...
        cartOrderItemVO.setTotalPrice(getTotalPrice(orderItemList));
        //返回结果
    ...
    public ServerResponse list(Integer userId,Integer pageNum,Integer pageSize) {
        ...
        if (orderList==null||orderList.size()==0){
            return ServerResponse.serverResponseByError("没有订单");
        }
    ...
    public ServerResponse detail(Long orderNo) {
       //参数的非空校验
        if (orderNo==null){
            return ServerResponse.serverResponseByError("订单编号为空");
        }
        //查询订单
    ...
    public ServerResponse send_goods(Long orderNo){
        ...
        //如果付款，则改为发货
        if (order.getStatus()!=Const.ORDER_STATUS.ORDER_PAYED.getCode()){
            return ServerResponse.serverResponseByError("未付款不能发货");
        }
        order.setStatus(Const.ORDER_STATUS.ORDER_SEND.getCode());
        int result = orderMapper.updateByPrimaryKey(order);
        if (result==0){
    ...
    public ServerResponse pay(Long orderNo) {
        ...
        return ServerResponse.serverResponseByError("支付错误");
    }
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        if (trade_status.equals(Const.TRADE_SUCCESS)){
            //支付成功
            //更改支付时间
            order.setStatus(Const.ORDER_STATUS.ORDER_PAYED.getCode());
        ...
        payinfo.setPayPlatform(PaymentPlatForEnum.ALIPAY.getCode());
        ...
        if (result>0){
            ...
            return ServerResponse.serverResponseBySuccess("回调成功");
        }
            return ServerResponse.serverResponseByError("支付信息返回失败");
    }

    @Override
    public void closeOrder(String time) {
            //根据创建时间查询订单列表，且未付款的订单
        List<Order> orderList = orderMapper.findOrderByCreateTime(Const.ORDER_STATUS.ORDER_UN_PAY.getCode(),time);
        if (orderList!=null&&orderList.size()>0){
            for (Order order:
                 orderList) {
                List<OrderItem> orderItemList = orderItemMapper.findByOrderNo(order.getOrderNo());
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        String subject = "订单："+order.getOrderNo()+"当面付扫码消费"+order.getStatus();
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl(Const.URL+"/LOLdaojucheng/Order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("E:\\Java_idea_workspace\\LOLdaojucheg\\src\\main\\webapp\\images/qr-%s.png",
